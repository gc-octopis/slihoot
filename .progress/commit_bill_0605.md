...
依使用者回報修正參與者端洗版警告 UX：原本 participant WebSocket 收到 rate-limit error 後會直接寫入共用 `error` banner，導致「禁止洗版」警告永久停在畫面上。

- 新增 `AutoDismissErrorBanner`：顯示 5 秒後自動清除，最後 400ms 做 fade-out。
- 參與者端新增獨立 `socketError` transient state，WebSocket error 不再覆蓋持久性頁面錯誤。
- 每次新的洗版 / rate-limit error 都會用新 id 重新觸發顯示週期，因此警告消失後，之後再次洗版才會再出現。
- 新增 `.error-banner.transient` / `.error-banner.transient.is-leaving` 淡出樣式，並以 `.error-stack` 同時容納持久錯誤與 transient socket 警告。
- 收窄 `.live-layout > .error-banner` selector，避免 stack 裡的 nested banner 繼承錯誤 grid-area。

#### 驗證（本次）

- `rtk bunx tsc --noEmit` 通過。
